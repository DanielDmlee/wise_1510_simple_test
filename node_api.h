 ...
 *  @returns NODE_API_OK on success
 */
unsigned short nodeApiInit(Serial *log_serial, Serial *sapi_serial);
 ...
 *
 *  Set Into Deep Sleep mode Wakeup timer with RTC
 *  @param millisec wakeup time in second
 *  @returns NODE_API_OK on success; others on failed
 */
unsigned short nodeApiSetDevSleepRTCWakeupMilli(int millisec);
